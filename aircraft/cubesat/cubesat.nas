...
var status_dialog = gui.Dialog.new("/sim/gui/dialogs/cubesat/status/dialog", "Aircraft/cubesat/Dialogs/status.xml");
var select_dialog = gui.Dialog.new("/sim/gui/dialogs/cubesat/select/dialog", "Aircraft/cubesat/Dialogs/select.xml");
var adjust_dialog = gui.Dialog.new("/sim/gui/dialogs/cubesat/adjust/dialog", "Aircraft/cubesat/Dialogs/adjust.xml");
...
var Model = {
	...
	add_derived_data : func(node) {
		...
		if (path == "Aircraft/cubesat/Models/sign.ac") {
			type = "OBJECT_SIGN";
			if (legend == "")
...
var modelmgr = {
	init : func(path) {
		me.active = nil;
		me.models = [];
		me.legendN = props.globals.getNode("/sim/gui/dialogs/cubesat-status/input", 1);
		me.legendN.setValue("");
		me.mouse_coord = geo.aircraft_position();
		me.import();
		me.marker = Model.new("Aircraft/cubesat/Models/marker.ac", geo.Coord.new().set_xyz(0, 0, 0));
		me.marker.hide();
		me.modelpath = path;
		if (me.active != nil)
			me.marker.move(me.active.pos);

		if (path != "Aircraft/cubesat/Models/cursor.ac")
			status_dialog.open();
	},
	...
	display_status : func(path) {
		...
		setprop("/sim/model/cubesat/status", "(" ~ size(me.models) ~ ")  " ~ path);
	},
	get_data : func {
		var n = props.Node.new();
		forindex (var i; me.models)
	...
	adjust : func(name, value, scale = 0) {
		...
		var cubesat = geo.aircraft_position();
		var dist = scale ? cubesat.distance_to(me.active.pos) * 0.05 : 1;
		if (name == "longitudinal") {
			var dir = cubesat.course_to(me.active.pos);
			foreach (var m; me.selected_models())
				m.apply_course_distance(dir, value * dist);

		} elsif (name == "transversal") {
			var dir = cubesat.course_to(me.active.pos) + 90;
			foreach (var m; me.selected_models())
				m.apply_course_distance(dir, value * dist);
...
var scan_dirs = func(csv) {
	var list = ["Aircraft/cubesat/Models/sign.ac", "Aircraft/cubesat/Models/cursor.ac"];
...
var collect_cubesat_data = func {
  var s = "\n\n------------------------------ CUBESAT -------------------------------\n";
	...
	s ~= sprintf("$ fgfs --aircraft=cubesat --lat=%.6f --lon=%.6f --altitude=%.2f --heading=%.1f\n",
			lat, lon, alt_ft, hdg);
	return s;
}
...
var print_data = func {
	...
	gui.popupTip("CUBESAT wrote model data to the console and the clipboard");
...
var export_data = func {
	var path = getprop("/sim/fg-home") ~ "/Export/cubesat-model-export.xml";
...
var export_flightplan = func {
	var path = getprop("/sim/fg-home") ~ "/Export/cubesat-flightplan-export.xml";
	var args = props.Node.new({ filename : path });
